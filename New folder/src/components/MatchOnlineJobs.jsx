// import React, { useEffect, useState } from 'react';
// import API from '../api/api';

// function MatchOnlineJobs() {
//   const [resumes, setResumes] = useState([]);
//   const [resumeId, setResumeId] = useState('');
//   const [query, setQuery] = useState('developer');
//   const [results, setResults] = useState([]);
//   const [loading, setLoading] = useState(false);

//   useEffect(() => {
//     const fetchResumes = async () => {
//       try {
//         const res = await API.get('resumes/');
//         setResumes(res.data);
//       } catch (err) {
//         console.error('Failed to fetch resumes', err);
//       }
//     };
//     fetchResumes();
//   }, []);

//   const handleMatch = async () => {
//     setLoading(true);
//     try {
//       const res = await API.post('match-online-jobs/', {
//         resume_id: resumeId,
//         query: query,
//       });
//       setResults(res.data.results || []);
//     } catch (err) {
//       console.error('Error matching online jobs:', err);
//     }
//     setLoading(false);
//   };

//   return (
//     <div className="p-6">
//       <h2 className="text-2xl font-bold mb-4">Match Resume with Online Jobs</h2>

//       <div className="mb-4 space-y-2">
//         <label className="block">
//           Select Resume:
//           <select
//             value={resumeId}
//             onChange={(e) => setResumeId(e.target.value)}
//             className="block w-full mt-1 p-2 border"
//           >
//             <option value="">-- Choose Resume --</option>
//             {resumes.map((resume) => (
//               <option key={resume.id} value={resume.id}>
//                 {resume.filename || resume.file.split("/").pop()}
//               </option>
//             ))}
//           </select>
//         </label>

//         <label className="block">
//           Job Title / Query:
//           <input
//             type="text"
//             value={query}
//             onChange={(e) => setQuery(e.target.value)}
//             placeholder="e.g. Python Developer"
//             className="block w-full mt-1 p-2 border"
//           />
//         </label>

//         <button
//           onClick={handleMatch}
//           disabled={!resumeId}
//           className="bg-green-600 text-white px-4 py-2 mt-2 rounded"
//         >
//           Match Jobs
//         </button>
//       </div>

//       {loading ? (
//         <p>Loading matching jobs...</p>
//       ) : (
//         results.length > 0 && (
//           <div>
//             <h3 className="text-xl font-semibold mb-4">Matching Jobs</h3>
//             {results.map((job, idx) => (
//               <div key={idx} className="border p-4 rounded mb-4 shadow">
//                 <h4 className="text-lg font-bold">{job.job_title}</h4>
//                 <p className="text-sm text-gray-600">{job.company}</p>
//                 <p className="mt-2 text-sm">Score: <strong>{job.score}%</strong></p>
//                 <p className="mt-1 text-sm">Matched Keywords: {job.matched_keywords.join(', ')}</p>
//                 <p className="mt-1 text-sm">Missing Keywords: {job.missing_keywords.join(', ')}</p>
//                 <a
//                   href={job.apply_link}
//                   target="_blank"
//                   rel="noopener noreferrer"
//                   className="mt-2 inline-block text-blue-600 underline"
//                 >
//                   Apply Now
//                 </a>
//               </div>
//             ))}
//           </div>
//         )
//       )}
//     </div>
//   );
// }

// export default MatchOnlineJobs;


import React, { useEffect, useState } from 'react';
import API from '../api/api';

function MatchOnlineJobs() {
  const [resumes, setResumes] = useState([]);
  const [resumeId, setResumeId] = useState('');
  const [query, setQuery] = useState('developer');
  const [results, setResults] = useState([]);
  const [loading, setLoading] = useState(false);

  useEffect(() => {
    const fetchResumes = async () => {
      try {
        const res = await API.get('resumes/');
        setResumes(res.data);
      } catch (err) {
        console.error('Failed to fetch resumes', err);
      }
    };
    fetchResumes();
  }, []);

  const handleMatch = async () => {
    setLoading(true);
    try {
      const res = await API.post('match-online-jobs/', {
        resume_id: resumeId,
        query: query,
      });
      setResults(res.data.results || []);
    } catch (err) {
      console.error('Error matching online jobs:', err);
    }
    setLoading(false);
  };

  return (
    <div
      className="min-h-screen bg-cover bg-center py-12 px-6"
      style={{
        backgroundImage: `url('https://static7.depositphotos.com/1106664/734/i/450/depositphotos_7349832-stock-photo-on-line-jobs.jpg')`,
      }}
    >
      <div className="max-w-4xl mx-auto bg-white bg-opacity-90 backdrop-blur-md p-8 rounded-lg shadow-xl">
        <h2 className="text-3xl font-bold mb-6 text-center text-gray-800">Match Resume with Online Jobs</h2>

        <div className="space-y-4">
          <div>
            <label className="block text-sm font-semibold text-gray-700 mb-1">Select Resume</label>
            <select
              value={resumeId}
              onChange={(e) => setResumeId(e.target.value)}
              className="w-full p-2 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-blue-400"
            >
              <option value="">-- Choose Resume --</option>
              {resumes.map((resume) => (
                <option key={resume.id} value={resume.id}>
                  {resume.filename || resume.file.split("/").pop()}
                </option>
              ))}
            </select>
          </div>

          <div>
            <label className="block text-sm font-semibold text-gray-700 mb-1">Job Title / Query</label>
            <input
              type="text"
              value={query}
              onChange={(e) => setQuery(e.target.value)}
              placeholder="e.g. Python Developer"
              className="w-full p-2 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-blue-400"
            />
          </div>

          <button
            onClick={handleMatch}
            disabled={!resumeId}
            className="w-full bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2 rounded transition duration-200 disabled:opacity-60"
          >
            {loading ? 'Matching...' : 'Match Jobs'}
          </button>
        </div>

        {results.length > 0 && (
          <div className="mt-10">
            <h3 className="text-2xl font-semibold mb-4 text-gray-800">Matching Jobs</h3>
            <div className="space-y-4">
              {results.map((job, idx) => (
                <div key={idx} className="bg-white p-5 rounded-lg shadow hover:shadow-md transition">
                  <h4 className="text-xl font-bold text-gray-800">{job.job_title}</h4>
                  <p className="text-sm text-gray-500">{job.company}</p>
                  <p className="mt-2 text-sm text-gray-700">Match Score: <strong>{job.score}%</strong></p>
                  <p className="mt-1 text-sm text-green-600">
                    Matched Keywords: {job.matched_keywords.join(', ')}
                  </p>
                  <p className="mt-1 text-sm text-red-500">
                    Missing Keywords: {job.missing_keywords.join(', ')}
                  </p>
                  <a
                    href={job.apply_link}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="inline-block mt-2 text-blue-600 hover:underline font-medium"
                  >
                    Apply Now →
                  </a>
                </div>
              ))}
            </div>
          </div>
        )}
      </div>
    </div>
  );
}

export default MatchOnlineJobs;

